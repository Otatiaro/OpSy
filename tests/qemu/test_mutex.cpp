 ...
 ******************************************************************************
 * @file    test_mutex.cpp
 * @brief   On-target tests for @c opsy::mutex ( @c priority_mutex ).
 ******************************************************************************
...
opsy::mutex g_plain;                                   // task-to-task only
opsy::mutex g_with_priority{ opsy::isr_priority{0x80} }; // also locks out ISRs
...
OPSY_QEMU_TEST(a_mutex_blocks_a_higher_priority_task_until_released)
{
	g_first_ran = 0;
	g_plain.lock();
...
OPSY_QEMU_TEST(a_mutex_can_be_relocked_after_release)
{
...
OPSY_QEMU_TEST(a_mutex_actually_excludes_two_contending_tasks)
{
	...
	CHECK(g_first.start([] { contend_on_mutex(); }, "contender-a"));
	CHECK(g_second.start([] { contend_on_mutex(); }, "contender-b"));
...
OPSY_QEMU_TEST(a_priority_carrying_mutex_reports_its_priority)
{
...
OPSY_QEMU_TEST(a_priority_carrying_mutex_locks_and_unlocks)
{
...
OPSY_QEMU_TEST(a_held_mutex_suspends_task_switching)
{
	g_second_ran = 0;
	g_stage      = 0;
...
OPSY_QEMU_TEST(time_keeps_advancing_while_a_mutex_is_held)
{
